...
class FlaneurGeocoder {
  ...
  _findFromGooglePlaceID(placeID) {
    return this._findPlaceDetails(placeID)
      .then((placeDetails) => {
        if (placeDetails) {
          return this._outputFromSingleGooglePlaceResult(placeDetails, { placeID: placeID })
        }
      ...
      .then((responseObject) => {
        if (responseObject.results) {
          ...
          if (localities.length === 1) {
            return this._outputFromSingleGooglePlaceResult(localities[0], { lat: lat, lng: lng })
          }
